...
- Where 条件过滤：
  - 选择指定表迁移时，可以为每张表配置过滤条件
  - 设置过滤条件后，只有满足过滤条件的数据才会被同步到目标库
  - 过滤条件需要满足 MySQL 的语法规则，如果过滤条件中包含引号，请使用单引号
  - 过滤条件中包含时间类型时，请使用 UTC 时区的时间
  - 对于 update 语句，需要更新前后的数据均满足过滤条件才会被同步到目标库
- 指定列脱敏：
  - 选择指定表迁移时，可以为每张表配置指定列脱敏
  - 设置指定列脱敏后，指定列的内容将不会被同步到目标库
  - 仅支持字符串类型字段脱敏
