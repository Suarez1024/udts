...
UDTS 支持 从 MySQL 迁移到 ClickHouse。
 MySQL 支持版本有 MySQL(包含Percona版)5.6/5.7/8.0/8.4，以及PolarDB(MySQL兼容版本)。
 ClickHouse 支持版本 23.8 至 24.8。

## 功能限制
1. 支持单库迁移，可迁移整库或指定表，不支持全库/多库迁移，不支持迁移存储过程、触发器、视图等。
2. 不支持 delete 语句。选择“自动创建库表”时，支持迁移 ddl 语句，详见“迁移内容--DDL”。
...
5. 选择“手动创建库表”时，UDTS 将不会在目标库创建库表，请您在目标库中提前创建待迁移库表，再启动迁移任务。增量/全+增任务选择“手动创建库表”时，不会同步 DDL 语句。
6. 选择“自动创建库表”时，如果待迁移的库表在目标库中不存在，UDTS 将在目标库中自动创建待迁移的库表；如果待迁移的库表在目标库中已经存在，UDTS 不会重新创建表结构。增量/全+增任务选择“自动创建库表”时，会自动同步增量运行期间产生的 DDL 语句。
...
| DDL      | CREATE/DROP/TRUNCATE TABLE <br> ALTER TABLE ADD/MODIFY/DROP COLUMN                                             |
| DML      | insert/update                                                                                                                    |
...
| 表名     | MySQL传输表名。 若不填，默认为迁移指定库中的所有表； 指定一张表传输， 请填写表名； 指定多张表传输，依次输入多张表名，表名之间使用英文逗号隔开    |
